...
var map, marker, geofences, watchID, circle, smallCircle, smallerCircle, smallerCircle2, speed, counter;
...
function initMap() {
  ...
  geofences = [
    ...
    {
      id: "geofence-safepurple",
      center: { lat: 39.0152, lng: -94.5467 },
      radius: 15, // in meters
      strokeColor: 'purple',
      strokeOpacity: 0.8,
      strokeWeight: 2,
...
smallerCircle2 = new google.maps.Circle({
  center: { lat: 39.0152, lng: -94.5467 },
  radius: 15, // in meters
  strokeColor: 'pink',
  strokeOpacity: 0.8,
...
google.maps.event.addListener(smallerCircle, 'radius_changed', function() {
  ...
  if (distance < smallerCircle.alertRadius) {
    ...
    // If the user is inside the alert radius, reset position of smallerCircle
    // Update the position of the small circle
    smallCircle.setCenter(new google.maps.LatLng(39.0135, -94.5495));
    alert(alertText);
    smallerCircle.setMap(null);
  }
});

// Add a listener to detect when the user enters the alert radius
google.maps.event.addListener(smallerCircle2, 'radius_changed', function() {
  var distance = google.maps.geometry.spherical.computeDistanceBetween(smallerCircle2.getCenter(), marker.getPosition());
  if (distance < smallerCircle2.alertRadius) {

    // If the user is inside the alert radius, reset position of smallerCircle
    // Update the position of the small circle
    smallCircle.setCenter(new google.maps.LatLng(39.0135, -94.5495));
    alert(alertText);
    smallerCircle2.setMap(null);
  }
});
