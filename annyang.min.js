...
function initMap() {
  ...
  geofences = [
    ...
    {
      id: "geofence-green",
      center: { lat: 39.0135, lng: -94.5455 },
      radius: 100, // in meters
      strokeColor: 'green',
      strokeOpacity: 0.3,
      strokeWeight: 4,
    ...
    },
    {
      ...
      fillOpacity: 0.35,
      alertRadius: 15,
      alertText: 'The ghost has vanished for now',
      script: 'document.getElementById("special-mask").checked = true;',
    },
  ];
 ...
 watchID = navigator.geolocation.watchPosition(function (position) {
  ...
  geofences.forEach(function (geofence) {
    ...
    if (distance <= geofence.alertRadius) {
      if (geofence.id === "geofence-safepink") {
        // Move geofence-safepink to a random location within 200 meters of geofence-brown
        ...
        geofence.center.lat += dy / 111111;
        geofence.center.lng += dx / (111111 * Math.cos(geofence.center.lat));
        // Update the circle's center
        geofence.circle.setCenter(geofence.center);
      } else {
...
if (distance != smallCircle.alertRadius) {
  ...
  else if (131 < distance && distance < 200) {
...
function moveSafepink() {
  var randomAngle = Math.random() * 2 * Math.PI;
  var randomDistance = Math.random() * 200;
  var dx = randomDistance * Math.cos(randomAngle);
  var dy = randomDistance * Math.sin(randomAngle);
  var newCenter = new google.maps.LatLng(
    safepink.circle.getCenter().lat() + dy / 111111,
    safepink.circle.getCenter().lng() + dx / (111111 * Math.cos(safepink.circle.getCenter().lat()))
  );
  safepink.circle.setCenter(newCenter);
}
