...
var map, marker, geofences, watchID, circle, smallCircle, bossGhost, smallerCircle, smallerCircle2, speed, counter;
...
function initMap() {
  ...
  geofences = [
    {
      id: "geofence-brown",
      center: { lat: 39.01492, lng: -94.5473 },
      radius: 30, // in meters
      strokeColor: 'brown',
      strokeOpacity: 0.3,
      strokeWeight: 4,
      ...
      script: "alert('hello from brown')",
    },
    {
      id: "geofence-green",
      center: { lat: 39.01469, lng: -94.5460 },
      radius: 30, // in meters
      strokeColor: 'green',
      strokeOpacity: 0.3,
      strokeWeight: 4,
      ...
      alertRadius: 30,
      alertText: 'You found the circle!',
      script: "alert('hello from green')",
    },
    {
      id: "geofence-safepink",
      center: { lat: 39.01475, lng: -94.5467 },
      radius: 20, // in meters
      strokeColor: 'pink',
      strokeOpacity: 0.8,
      strokeWeight: 2,
      ...
      fillOpacity: 0.35,
      alertRadius: 20,
      alertText: 'You have feared away the spirits',
        script: "speed = -25;setTimeout(function() { speed = 5; }, 15000);var randomAngle = Math.random() * 2 * Math.PI;var randomDistance = Math.random() * 40;var dx = randomDistance * Math.cos(randomAngle);var dy = randomDistance * Math.sin(randomAngle);geofence.center.lat += dy / 111111;geofence.center.lng += dx / (111111 * Math.cos(geofence.center.lat));geofence.circle.setCenter(geofence.center);",
    },
  ];

  // Create the geofences
  geofences.forEach(function (geofence) {
    geofence.circle = new google.maps.Circle({
      id: geofence.id,
      ...
      alertRadius: geofence.alertRadius,
      alertText: geofence.alertText,
      script: geofence.script,
      map: map
    });
  });
...
var newPosition = google.maps.geometry.spherical.computeOffset(smallCircle.getCenter(), 4, google.maps.geometry.spherical.computeHeading(smallCircle.getCenter(), marker.getPosition()));
...
if (distance != smallCircle.alertRadius) {
  ...
  if (0 < distance && distance < 8) {
    document.getElementById('sentence').innerHTML = 'Ill make sure to write your eulogy';
    setTimeout(function() { window.location.href = 'death.html'; }, 4000);
    smallCircle.setOptions({
    strokeOpacity: 1,
    fillOpacity: 1
  });
  }

  else if (9 < distance && distance < 45) {
    document.getElementById('sentence').innerHTML = 'We are about to die';
    document.getElementById('special-mask').checked = true;
    smallCircle.setOptions({
    strokeOpacity: 0.8,
    fillOpacity: 0.7
  });
  ...
  else if (46 < distance && distance < 130) {
    document.getElementById('sentence').innerHTML = 'I see enemies all around us, why are we still here';
    document.getElementById('mask-emotions--shocked').checked = true;
    smallCircle.setOptions({
    strokeOpacity: 0.4,
    fillOpacity: 0.2
  });
  ...
  else if (131 < distance && distance < 300) {
    document.getElementById('mask-emotions--sad').checked = true;
    document.getElementById('sentence').innerHTML = 'You have awakened the ghosts';
    smallCircle.setOptions({
    strokeOpacity: 0.2,
    fillOpacity: 0.1
  });
...
function moveSmallCircle() {
  ...
  setInterval(function () {
    if (!bossGhost) {
      // Get the center of the first geofence
var geofenceCenter = geofences[1].center;

// Create the small circle
bossGhost = new google.maps.Circle({
  center: geofenceCenter,
  radius: 8, // in meters
  strokeColor: 'red',
  strokeOpacity: 0,
...
// Move the small circle towards the user's location
var newPosition = google.maps.geometry.spherical.computeOffset(bossGhost.getCenter(), speed, google.maps.geometry.spherical.computeHeading(bossGhost.getCenter(), marker.getPosition()));
bossGhost.setCenter(newPosition);

// Check the distance between the user and the small circle
var distance = google.maps.geometry.spherical.computeDistanceBetween(bossGhost.getCenter(), marker.getPosition());

if (distance != bossGhost.alertRadius) {

  if (0 < distance && distance < 8) {
    document.getElementById('sentence').innerHTML = 'Ill make sure to write your eulogy';
    setTimeout(function() { window.location.href = 'death.html'; }, 4000);
    bossGhost.setOptions({
    strokeOpacity: 1,
    fillOpacity: 1
  });
  }
